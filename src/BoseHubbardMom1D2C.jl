...
function Rimu.dimension(h::TwoComponentHamiltonian)
...
function Rimu.starting_address(h::BoseHubbardMom1D2C)
    return BoseFS2C(starting_address(h.ha), starting_address(h.hb))
end

Rimu.LOStructure(::Type{<:BoseHubbardMom1D2C{<:Real}}) = IsHermitian()
...
function Rimu.num_offdiagonals(ham::BoseHubbardMom1D2C, address::BoseFS2C)
    M = num_modes(address)
...
function Rimu.diagonal_element(ham::BoseHubbardMom1D2C, address::BoseFS2C)
    M = num_modes(address)
...
function Rimu.get_offdiagonal(
    ham::BoseHubbardMom1D2C, address::BoseFS2C, chosen
)
...
function Rimu.offdiagonals(h::BoseHubbardMom1D2C{T}, a::BoseFS2C) where {T}
