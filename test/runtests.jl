...
@testset "RimuLegacyHamiltonians.jl" begin
    ...
    @testset "BoseFS2C 2" begin
        ...
        for address in (
            ...
            @test single_particle_density(DVec(address => 1); component=2) == (0, 1, 0)
        end
    ...
    @testset "G2MomCorrelator" begin
        ...
        @test eval(Meta.parse(repr(G2MomCorrelator(2, :first)))) == G2MomCorrelator(2, :first)
        @test eval(Meta.parse(repr(G2MomCorrelator(2, :second)))) == G2MomCorrelator(2, :second)
